...
Hey there 👋  so you want to process some events from the past 🕓, probably a time when this plugin wasn't installed on your Moodle. That's awesome, and we really want to help with 
...
WHERE L.userid IN (
  ...
  FROM mdl_enrol  AS E
         INNER JOIN mdl_user_enrolments AS UE
                    ON E.id = UE.enrolid
  WHERE ( E.courseid = 123 OR E.courseid = 456 OR E.courseid = 789) AND L.userid>2
    AND L.eventname IN (
